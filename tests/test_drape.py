...
def drape(get_yaml, build_blade, build_structure, plybook):
    ...
    suff = ("_web.vtp", "shell", "_shell1.vtu")
    draped_mesh = drape_mesh.drape_mesh(
        pref + suff[0], plybook, suff[1], pref + suff[2]
    )
    return draped_mesh  # build_plybook.lamplan2plies(get_yaml)


def test_drape_mesh_basic(drape):
    assert drape.n_points == 4000
    assert drape.n_cells == 3960
    assert drape.points[3000] == pytest.approx([-1.0641935, 1.800456, 75.757576])
    assert list(
        drape.cell_data
...
def test_drape_slab_thicknesses(drape):
    max_thicknesses = [drape.cell_data[k].max() for k in skeys]
    assert max_thicknesses == pytest.approx([30.0, 5.0, 42.34, 50.0, 13.799999, 111.25])


def test_drape_slab_volume(drape):
    volumes = [(drape.cell_data[k] * drape.cell_data["Area"]).sum() for k in skeys]
