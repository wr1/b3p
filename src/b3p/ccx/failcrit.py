...
def failure_calc(
    ...
    Vectorized using NumPy for multiple stress states.

    Parameters:
    - STRESS: Array of stress tensors, shape (n, 6) [σ1, σ2, σ3, τ23, τ13, τ12] (MPa)
    - UPSTRAN: Array of strains, shape (n, 6) (not used in calculation, included for compatibility)
    ...
    # Initialize IFF index and critical angle
    IFF_max = np.zeros(n)

    # Loop over fracture plane angles (-90° to 90°)
    for J in range(181):  # 0 to 180 corresponds to -90° to 90°
        ...
        # Stresses on fracture plane (vectorized for all stress states)
        SFP = (
            STRESS[:, 1] * cos_theta2
            + STRESS[:, 2] * sin_theta2
            + 2.0 * STRESS[:, 5] * sin_cos_theta
        )

        TNT = (
            -STRESS[:, 1] * sin_cos_theta
            + STRESS[:, 2] * sin_cos_theta
            + STRESS[:, 5] * (cos_theta2 - sin_theta2)
        )

        TN1 = STRESS[:, 3] * cos_theta + STRESS[:, 4] * sin_theta
        ...
        # Update maximum IFF and corresponding angle
        update_mask = IFF > IFF_max
        IFF_max = np.where(update_mask, IFF, IFF_max)
        THETAMAX = np.where(update_mask, THETA / deg2rad, THETAMAX)
