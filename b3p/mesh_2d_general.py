...
def get_connectivity_counts(sec):
    con = sec.lines
    nn1 = np.concatenate([con[1::3], con[2::3]])
    nodes, counts = np.unique(nn1, return_counts=True)
    return nodes, counts
...
def cut_blade(r, vtu, if_bondline=True, rotz=0, var=None, is2d=False, verbose=False):
    ...
    rotated_section = sec.rotate_z(rotz)

    pts = rotated_section.points

    minxy = np.amin(pts, axis=0)
    maxxy = np.amax(pts, axis=0)
    mid_position = 0.5 * (minxy + maxxy)

    nply = sec.cell_data["n_plies"]
    thickness = sec.cell_data["thickness"]
    enormal = sec.cell_data["Normals"]

    # gather cell-wise ply arrays
    plies = [sec.cell_data[i] for i in sec.cell_data if i.startswith("ply")]

    # get the number of times nodes are linked to by elements

    # get the connectivity
    nodes, counts = get_connectivity_counts(sec)

    print(nodes, counts)

    meshed_stacks = []
    for i in range(sec.n_cells):  # loop over each cell
        cell = sec.get_cell(i)
        i1, i2 = (cell.point_ids[0], cell.point_ids[1])

        n1, n2 = (sec.point_data["Normals"][i1], sec.point_data["Normals"][i2])
        nc = sec.cell_data["Normals"][i]
        t = np.array([p[i] for p in plies if p[i][1] > 1e-4]) * 1e-3

        cumulative_thickness = np.concatenate([[0], np.cumsum(t[:, 1])])
        pts1 = cell.points[0] + np.outer(cumulative_thickness, nc)
        pts2 = cell.points[1] + np.outer(cumulative_thickness, nc)

        lp1 = len(pts1)

        poly = pv.PolyData(np.concatenate([pts1, pts2]))

        meshed_stacks.append(poly)

        print(lp1)

    merged_mesh = pv.merge(meshed_stacks)

    merged_mesh.save("merged_mesh.vtp")

    print(nodes[counts == 3])
